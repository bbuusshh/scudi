...
class NIXSeriesDigitalOutput(ProcessSetpointInterface):
    """ A module to output and read back (internally routed) digital voltages
    ...
    _digital_channels = ConfigOption(
        name='digital_channels',
        default={
            'do0': {'pulse': False, 'dtype': float, 'keep_value': False},
            'do1': {'pulse': False, 'dtype': float, 'keep_value': False},
        },
        missing='info'
    )
    ...
    def on_activate(self):
        ...
        avlbl_do_ch_names = tuple(
            term.rsplit('/', 1)[-1].lower() for term in self._device_handle.do_physical_chans.channel_names)
        ao_ch_names_set = set(self._extract_ch_name(ao_ch_name) for do_ch_name in self._setpoint_channels)
