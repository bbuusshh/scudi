...
import numpy as np
from TimeTagger import createTimeTagger, Dump, Correlation, Histogram, Counter, CountBetweenMarkers, FileWriter, Countrate, Combiner, TimeDifferences, Flim, CHANNEL_UNUSED
from qudi.core.configoption import ConfigOption
from qudi.core.module import Base


class TimeTaggerHW(Base):
    ...
    _combiner = ConfigOption('combiner', False, missing='warn')
    _flim = ConfigOption('flim', False, missing='warn')
    ...
    def setup_TT(self):
        ...
        self._combined_channels = self.combiner(self._click_channels["channels"])
    ...
    def histogram(self, **kwargs):  
        ...
        Example:
        channel=1, trigger_channel=5, bin_width=1000, numer_of_bins= 1000
    ...
    def correlation(self, **kwargs):  
        ...
        Besides, it is possible to set values:
        Example:
        channel_start=1, channel_stop=2, bin_width=1000, numer_of_bins= 1000
    ...
    def counter(self, **kwargs):
        ...
        for key, value in kwargs.items():
            ...
            if key == 'refresh_rate' and value != None:
                self._counter['bin_width'] = int(1e12/value)
        return Counter(self.tagger,
                                self._counter['channels'],
                                self._counter['bin_width'],
    ...
    def flim(self, **kwargs):
        '''Returns a Flim TimeTagger object
        kwargs: n_pixels, n_bins, binwidths 
        Flim(tagger, start_channel, click_channel, pixel_begin_channel, n_pixels, n_bins, binwidth[, pixel_end_channel=CHANNEL_UNUSED, frame_begin_channel=CHANNEL_UNUSED, finish_after_outputframe=0, n_frame_average=1, pre_initialize=True])
        '''
        for key, value in kwargs.items():
                self._flim.update({key:int(value)})

        self._flim['click_channel'] = self._combined_channels.get_channel()

        _flim_instance = Flim(self.tagger, 
                    self._flim['start_channel'], 
                    self._flim['click_channel'], 
                    self._flim['pixel_begin_channel'], 
                    self._flim['n_pixels'], 
                    self._flim['n_bins'], 
                    self._flim['binwidth'], 
                    pixel_end_channel=CHANNEL_UNUSED, 
                    frame_begin_channel=CHANNEL_UNUSED, 
                    finish_after_outputframe=1, 
                    n_frame_average=1, 
                    pre_initialize=True)

        return _flim_instance
