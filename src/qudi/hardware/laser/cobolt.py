...
class Cobolt(Base):
    ...
    def enable_modulated(self):
        """ ENABLE the laser.
        """
        if self.portOK:
            self.port.write((f"sdmes 1 {EOL}").encode())
    
    def disable_modulated(self):
        """ ENABLE the laser.
        """
        if self.portOK:
            self.port.write((f"sdmes 0 {EOL}").encode())

    def set_laser_modulated_power(self, power):
        if self.portOK:
            self.port.write((f"slmp {power} {EOL}").encode())

    def enable(self):
        """ ENABLE the laser.
        """
        if self.portOK:
            self.port.write((f"l 1{EOL}").encode())
            
    def disable(self):
        """ Disable the laser.
        """
        if self.portOK:
            self.port.write((f"l 0{self.channel}{EOL}").encode())
