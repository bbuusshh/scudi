...
class verdiLaser:
    ...
    def __init__(self, ioport='/dev/ttyUSB0'):
        ...
        """
        # configure the port
        self.port = serial.Serial(port = ioport, 
                                    baudrate = 19200, 
        ...
        if self.portOK:
            self.port.write(("E = 0%s" % EOL).encode())   # echo off
            time.sleep(0.1)
            self.port.write(("> = 0%s" % EOL).encode())   # prompt off
            time.sleep(0.1)
    ...
    def standbyON(self):
        ...
        """
        if self.portOK:
            self.port.write(("L = 0%s" % EOL).encode())
    
    def enableON(self):
        """ ENABLE the laser.
        """
        if self.portOK:
            self.port.write(("L = 1%s" % EOL).encode())
        
    def getShutter(self):
        """ Check the state of the shutter.
            returns an int: 0 - shutter is closed; 1 - shutter is open
        """
        return int(self.laserQuery('S'.encode()))
    ...
    def setShutter(self, state):
        ...
        if self.portOK:
            if state in (1, 0, "ON", "OFF"):
                self.port.write(("S = %s%s" % (state, EOL)).encode())
    ...
    def setVanadateTemp(self, vanadateTemp):
        ...
        if self.portOK:
            if vanadateTemp > 45:
                ...
                vanadateTemp = 20.00
            self.port.write(("VT = %s%s" % (vanadateTemp, EOL)).encode())
