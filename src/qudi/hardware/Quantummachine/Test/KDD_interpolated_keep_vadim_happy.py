...
def sequence_generator(error, N_pulses, taus):
    ...
    for i in range(N_pulses):
        m += error
        if np.abs(m <= 0.5):
            U.append(int(taus[0])//4)
        else:
            U.append(int(taus[1])//4)
            m -= 1
    return U
...
# sequence = [int(tau)//4 for tau in tau_sequence for tau_sequence in sequence]
#
# real_taus = [int(tau)//4 for tau in tau_sequence for tau_sequence in real_taus]
