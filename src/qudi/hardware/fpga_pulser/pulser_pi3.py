...
from qudi.hardware.fpga_pulser import pulser_client
...
class PulseStreamer(PulserInterface):
    ...
    """
    __pulser_client = Connector(name='pulser_client', interface='PulserClient')
    ...
    def on_activate(self):
        ...
        self._pulser_client = self.__pulser_client()
        self.__samples_written = 0
    ...
    def load_waveform(self, load_dict):
        ...
        self._cf = self.__current_waveform
        self._seq = self.convert_to_pi3pulser_sequence(self.__current_waveform)
        self._pulser_client.set_sequence(self._seq)
    ...
    def convert_to_pi3pulser_sequence(self, wavefront):
        len_tot = sum([dt[0] for dt in list(wavefront.items())[0][1]])
        wf = np.zeros((len(wavefront.keys()), len_tot))
        for ch, sequence in wavefront.items():
            start_sq = 0
            ch_num = int(''.join(c for c in ch if c.isdigit())) - 1
            for seq in sequence:
                wf[ch_num][start_sq:start_sq + seq[0]] = np.ones(seq[0]) * seq[1]
                start_sq += seq[0]

        next_seqs = np.unique(np.where(np.abs(np.diff(wf.T, axis=0)) > 0)[0])
        next_seqs = np.append(next_seqs, len_tot - 1)
        seq_coords = next_seqs
        next_seqs = np.append(next_seqs[0] + 1, np.diff(next_seqs))

        seq_ = []
        for idx, step in enumerate(next_seqs):
            seq_.append(tuple([[f'ch{num}' for num in range(wf.T.shape[1]) if wf.T[seq_coords[idx]][num] ], step]))
        return seq_