...
class magnet_3d(Base):
    ...
    magnet_x = Connector(interface='CoilMagnet1DInterface')
    magnet_y = Connector(interface='CoilMagnet1DInterface')
    magnet_z = Connector(interface='CoilMagnet1DInterface')
...
# stuff that is used in the functions above
    ...
    def _ramp_loop(self, field_target=[None,None,None], current_target=[None,None,None], enter_persistent=False, first_time=False):
        # TODO: Can we shrink this monstrosity?
        print('_ramp_loop')
        ...
        # If we now decide to ramp to 0.1T and call the ramp() function, it would unpause the ramp. 
    ...
    def _ramp_supply(self, current_target):
        ...
        It is used to adjust the supply current to the magnet current before turning on the PSW heaters.
        """
