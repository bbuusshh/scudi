...
class PowerControllerLogic(LogicBase):
    ...
    sig_set_power = QtCore.Signal(float, int)
    ...
    def on_activate(self):
        ...
        self.sig_set_power.connect(self.set_power)
    ...
    def calibrate_power_wheel(self, motor=1):
        ...
        delay(2000)
        self._motor_pi3.moveToAbsolutePosition(motor=motor, pos = 0)
        self.power_calib = np.vstack((angles, np.array(powers))).T
        self.power_calib = self.power_calib[np.argsort(self.power_calib[:,1])]
        self._power_range = (min(self.power_calib[:,1]), max(self.power_calib[:,1]))

        # self._motor_pi3.stopAllMovements()
    
    @QtCore.Slot(float, int)
    def set_power(self, power, motor=0):
        if (power > self._power_range[1]) or (power < self._power_range[0]):
            print("Power out of calibrated range")
            return -1
        new_angle = self.power_calib[:, 0][np.argmin(np.abs(self.power_calib[:, 1] - power))]
    ...
    def run_saturation(self, motor = 0):
        ...
        counts = []
        for angle in np.sort(self.power_calib[:, 0]):
            self._motor_pi3.moveToAbsolutePosition(motor = motor, pos = angle)
            delay(2000)
        ...
        self._saturation_data = np.vstack(( self.power_calib[:, 1], np.array(counts))).T
        self.sig_data_updated.emit()
